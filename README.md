...
- 搜索描述、转写、摘要、标签和关键观点；
- 查看单条视频的完整知识内容；
- 支持 CLI 文本、Markdown、JSON 输出和前端只读 HTTP API。
...
当前开始前端建设。多用户、知识图谱和复杂推荐暂不优先。

## Docker 配置

项目通过 `.env` 读取 MySQL、Redis、Faster-Whisper、DeepSeek 和 HTTP API 配置。
...
当前优先级切换为前端产品开发；除前端对接所需接口外，后端扩展计划暂时暂停。
